...
func (a *VivaldiAdapter) OpenInProfile(ctx context.Context, profile string, payloadToken string) (OpenResult, error) {
	...
	// IDEMP (SSOT §6.6 "既存があれば focus、無ければ作る") against an IN-FLIGHT
	// first-run launch. When a managed Vivaldi process is already alive (a prior
	// converge-loop replan's `open` whose window is still being generated by a
	// cold first-run profile), re-issuing `open` would NOT create a second
	// window — Chromium is single-instance per --user-data-dir — but it would
	// re-run the session purge and add latency for nothing. Worse, the
	// converge loop's repeated spawn-browser must WAIT on the in-flight window,
	// not relaunch. So when the managed process is in-flight we skip the launch
	// and proceed straight to the diff settle, which catches the same window
