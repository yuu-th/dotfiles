...
var ssotLedger = []ledgerItem{
	...
	{ID: "OP-13", Section: "§4.1/§10.6", Layer: "L0/L1/L2/L3/L4", Subject: "remove-window removes desired identity and closes/kills target + project keeps empty Windows on last removal. 2026-05-30: fixed a same-title-sibling hole — removing one of two same-basename editors (e.g. editor-2) previously left the window open (planner over-protected the ambiguous sibling + executor PreUniqueStrong re-resolved the deleted desired); now uses provenance to close the orphan safely without touching the active editor (TestRemoveWindow_ClosesRemovedWindow).", TestPath: "internal/reducer/reducer_v3_test.go internal/planner/planner_remove_window_test.go internal/controller/ssot_removewindow_test.go scenarios/ssot_real_acceptance_test.go", TestName: "TestReduceIntent_RemoveWindow/TestPlanner_RemoveWindow_EmitsCloseOpForRemovedShell/TestPlanner_RemoveWindow_LastWindowKeepsProject/TestRemoveWindow_ClosesRemovedWindow/TestHumanE2ESSOTRemoveWindowClosesAndKillsSessionSteps", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "SYS-ALL", Section: "§4.2/§10.6", Layer: "L1/L2/L3/L4", Subject: "move-window-to-workspace, reorder-columns, close-window, spawn-* (all kinds), kill-session — real_ops 実機 PASS 2026-05-31 (each §4.2 op verified by its L3 behavior owner in one safe session: move M1-2, reorder R1-4, close C1/C4/C5, spawn S1-S11, kill-session T4; L4 composition is owned by ACC-S1/S5/S9)", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go internal/adapter/session/ssot_l3_session_real_ops_test.go", TestName: "", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "L3-S", Section: "§10.4", Layer: "L3", Subject: "spawn real operation table S1-S11 (S7 TestSpawnBrowserAlreadyExists honest t.Skip — BR-EXIST needs a Vivaldi profile-aware querier; tracked separately)", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go internal/adapter/wm/ssot_l3_wm_real_ops_test.go", TestName: "", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "L3-M", Section: "§10.4", Layer: "L3", Subject: "move real operation table M1-M2", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestMoveToWorkspace/TestMoveToWorkspaceAlreadyOnTarget", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "L3-R", Section: "§10.4", Layer: "L3", Subject: "reorder real operation table R1-R4 (external-window tolerance fixed: managedOrderSettled)", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestReorderColumns/TestReorderColumnsAlreadyCorrect/TestReorderColumnsPartialMatch/TestReorderColumnsEmptyWorkspace", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "L3-C", Section: "§10.4", Layer: "L3", Subject: "close real operation table C1/C4/C5", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestLifecycleRemovalPrimaryCloseSurfaces/TestCloseWindowAlreadyGone/TestCloseCockpit", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "L3-F", Section: "§10.4", Layer: "L3", Subject: "focus real operation table F1-F4", TestPath: "internal/adapter/wm/ssot_l3_wm_real_ops_test.go internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestRealOpsFocusWorkspace/TestRealOpsFocusWorkspaceNonExistent/TestFocusWindow/TestFocusWindowVanished", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "L3-B", Section: "§10.4", Layer: "L3", Subject: "startup recovery table B1-B4 (B3 unparseable-ghostty-orphan t.Skip — omniwm ghostty titleRegex filter, see L3-I)", TestPath: "internal/controller/ssot_l3_startup_real_ops_test.go", TestName: "TestStartupNormal/TestStartupMissingWindow/TestStartupOrphanWindow/TestStartupStateCorrupted", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "L3-U", Section: "§10.4", Layer: "L3", Subject: "scratch shell show/hide real operation U1 (omniwm app-rule projwm-scratch-shell added so the window is catalogued)", TestPath: "internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestScratchShellShowReturnsLiveWindowID/TestScratchShellShowHideRestoresPriorFocus", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	// §9.1 L4 acceptance.
	{ID: "ACC-S1", Section: "§9.1", Layer: "L4", Subject: "SwitchProfile: close old windows, summon new (実機 PASS incl. browser via B-05 fix)", TestPath: "scenarios/ssot_l4_acceptance_spec_test.go scenarios/real_acceptance_test.go", TestName: "TestSSOTL4S1SwitchProfile", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "ACC-S2", Section: "§9.1", Layer: "L4", Subject: "ArchiveProject: close windows and kill tmux (実機 PASS 2026-05-31; browser respawn-loop 根治 commit 1f47390 — payload GC を archive→delete に + OpenInProfile inFlight skip 撤去。両 sub-test Archive+ProductionRemoval green)", TestPath: "scenarios/ssot_l4_acceptance_spec_test.go scenarios/real_acceptance_test.go", TestName: "TestSSOTL4S2ArchiveProject", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "ACC-S3", Section: "§9.1", Layer: "L4", Subject: "UnarchiveProject: returns to park state (実機 PASS)", TestPath: "scenarios/ssot_l4_acceptance_spec_test.go scenarios/real_acceptance_test.go", TestName: "TestSSOTL4S3UnarchiveProject", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "ACC-S4", Section: "§9.1", Layer: "L4", Subject: "Assign/Unassign: slot assignment and release (実機 PASS)", TestPath: "scenarios/ssot_l4_acceptance_spec_test.go scenarios/real_acceptance_test.go", TestName: "TestSSOTL4S4AssignUnassign", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "ACC-S5", Section: "§9.1", Layer: "L4", Subject: "Reconcile: repairs diff (実機 PASS)", TestPath: "scenarios/ssot_l4_acceptance_spec_test.go scenarios/real_acceptance_test.go", TestName: "TestSSOTL4S5Reconcile", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "ACC-S7", Section: "§9.1", Layer: "L4", Subject: "OmniWM restart recovery (実機 PASS×2; reorder 振動→絶対 move-column-to-first + per-move 観測の非致命化、final-focus を最終 layout op 化)", TestPath: "scenarios/ssot_real_acceptance_test.go internal/adapter/wm/ssot_l3_wm_spec_test.go internal/planner/planner_test.go", TestName: "TestHumanE2ESSOTOmniWMRestartRecoverySteps/TestReorderColumnsStackedFiveWindowRotation/TestReorderColumnsWithStackedColumn", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "ACC-S10", Section: "§9.1", Layer: "L4", Subject: "tmux/Ghostty/Zed crash recovery — REAL-VERIFIED 2026-06-08 (Ghostty 6.1s / tmux 9.7s / Zed 26.9s recovery, all within §9.2③ 1m budget). Zed-crash step requires a Zed-free session (attrZedMainProcCount==1 sole-Zed safety gate, else honest-skip): verified by booting out projwmd-next + the 4 event watchers + safety-timer and killing the existing managed Zed so the test's spawned editor is the sole instance; Ghostty/tmux steps run unconditionally.", TestPath: "scenarios/ssot_real_acceptance_test.go", TestName: "TestHumanE2ESSOTCrashRecoverySteps", Status: statusRealOnly, Evidence: evidenceBehavior},
	...
	{ID: "ZED-CONFIG", Section: "§4.4 editor / §10.9 GAP-07", Layer: "L0/L3", Subject: "managed Zed config (SETTINGS) separation: launch always uses `-n` (new window via the Zed CLI shim, not reuse) + `--user-data-dir <private>` whose settings disable restore_on_startup and auto_install_extensions. NOTE (2026-05-30 real-machine): `--user-data-dir` only separates SETTINGS for the first-launched process; Zed is single-instance (GPUI) so it does NOT give PROCESS isolation from the user's Zed — window attribution/safety is owned by ZED-ATTR (provenance), not this config. Pre-existing empty-project protection is the L3 owner (TestSpawnEditorEmptyProjectCleanup)", TestPath: "internal/adapter/wm/sigwm_zed_config_test.go internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestZedManagedSettingsIsolatesFromUserZed/TestZedLaunchArgsForceNewWindowAndPrivateDataDir/TestSpawnEditorEmptyProjectCleanup", Status: statusRealOnly, Evidence: evidenceBehavior},
	{ID: "ZED-ATTR", Section: "§6.9 / §6.9.1 (ATTR-*)", Layer: "L0/L1/L2/L3", Subject: "managed-window provenance attribution (single-process safety) — IMPLEMENTED 2026-05-30. Extends INV-01 (uniqueness) + INV-10 (identity-from-title) + INV-11 (external untouched): Zed is single-instance (no process attribution, see ZED-CONFIG), so the managed window is owned by PROVENANCE (live window-ID projwm spawned/adopted; a validated cache re-checked each observe via ResolveWithOptions provenance pre-pass + pruneProvenance), and a non-provenance same-title window stays External — never moved/closed/killed. Cold-start/recovery adopts by title→identity ONLY within slot territory (adoptableOnSlot); user-workspace windows inviolable. Persisted in ControllerCheckpoint.WindowProvenance (G1). Check14 provenance-aware. Deterministic L0/L1/L2 (24 unit tests) GREEN. L3 real_ops REAL-VERIFIED 2026-05-30 (safe session = daemon bootout + 0 Zed): F1/A1/D3 PASS — F1 confirms removing a managed Zed window keeps the PROCESS alive (single-process safety, the pid-kill-prevention guarantee); D2 honest-skip (bare-Zed empty-project setup race). L4 humanE2E (B1/B3/A2/F1) REMOVED 2026-06-07 as a SSOT layer齟齬 fix: §6.9.1 assigns these attribution edge-cases to L0/L2 (F1 to L3), and §10.2 states a single contract verified at L2/L3 is NOT re-tested at L4 (L4 = composite scenarios / fault recovery only). The L4 duplicates required spawning a *distinguishable* real user Zed window — the exact single-process limitation §6.9.1 receives as 原理的に不可能 (⚠️ A5/B5/C2, 防御せず受容) — so they could not pass and were a drift from the spec's layer strategy. The ATTR-F2 single-process safety PRECONDITION helpers (zedMainPIDs/attrZedMainProcCount) moved to scenarios/attr_f2_zed_safety_test.go (used by S10's gated Zed-crash step). 2026-06-07 re-verification (dedicated Zed-free session, projwm event pipeline stopped): L3 F1/A1 PASS, D3 PASS, D2 honest-skip — confirming the spec-designated L3 owners. Accepted limitations (doc-only): ATTR-A5/B5/C2/H1. Folded dups: ATTR-D1≡ZED-CONFIG empty-project, ATTR-A6 also≡L3-S5. ATTR-D4 IMPLEMENTED 2026-06-05: closeNewZedEmptyProjects now takes the just-spawned project window's live id as `protect` and never closes it even at a transient empty title; deterministic L2 owner TestCloseNewZedEmptyProjects_ProtectsLoadingProjectWindow (mock-executor close decision, no racy real empty-title moment needed). FLAGGED SEPARATE BUG: RemoveWindow executor close-precondition fails after reducer deletes the desired window (executor.validateLiveUniqueStrong → desired-window-unknown), provenance-independent, out of ATTR scope.", TestPath: "internal/identity/ssot_attr_test.go internal/controller/ssot_attr_test.go internal/controller/ssot_attr_lifecycle_recovery_test.go internal/invariant/ssot_attr_l1_test.go internal/adapter/wm/ssot_attr_real_ops_test.go internal/adapter/wm/ssot_attr_real_ops_extra_test.go", TestName: "TestZedAttr_A2_ProvenancePrecedenceOverCollidingTitle/TestZedAttr_A3_StaleProvenanceIDNotReported/TestZedAttr_A4_ProvenanceIDReusedByDifferentApp/TestZedAttr_B4_ProvenanceUniqueAmongThreeCollidingTitles/TestZedAttr_C1_ProvenanceDistinguishesMultipleEditors/TestZedAttr_B3_UserWindowOnOwnWorkspaceUntouched/TestZedAttr_B1_UserOpensSameNameAfterOwnership/TestZedAttr_A1_SpawnRecordsProvenance/TestZedAttr_A3_RespawnAfterOurWindowGone/TestZedAttr_B2_AdoptSameTitleOnSlotNoDuplicate/TestZedAttr_C1_TwoEditorsSameProjectBothManaged/TestZedAttr_E1_RemoveWindowClearsProvenance/TestZedAttr_A6_SpawnDedupProvenanceStable/TestZedAttr_E2_ArchiveClearsManagedEditor/TestZedAttr_E3_ProfileSwitchClosesLeavingEditor/TestZedAttr_E4_SlotActivationSpawnsEditor/TestZedAttr_E5_SlotDeactivationHandlesEditor/TestZedAttr_G1_DaemonRestartNoRespawn/TestZedAttr_G2_RebootStaleProvenanceFreshSpawn/TestZedAttr_G3_RebootUserZedUntouchedColdStart/TestZedAttr_D5_ConcurrentMultiSpawnNoCrossContamination/TestZedAttr_B4_ProvenanceAwareCheck14DoesNotFlagUserWindow/TestZedAttr_B4_Check14StillFiresWithoutProvenance/TestZedAttr_F1_RemoveManagedWindowKeepsProcessAlive/TestZedAttr_A1_SpawnYieldsStableObservableID/TestZedAttr_D2_PreExistingEmptyProjectUntouched/TestZedAttr_D3_DelayedStrayDoesNotBreakReorder/TestCloseNewZedEmptyProjects_ProtectsLoadingProjectWindow", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "CLI-EFFECT", Section: "§5.7 / §10.9 GAP-14", Layer: "L1", Subject: "every writable CLI command constructs + submits the correct intent from its args (not just usage surface): archive/unarchive→Archive/UnarchiveProject, profile create/delete/switch/rename/assign/unassign→Create/Delete/Switch/Rename/AssignProject/UnassignSlot, reconcile→Reconcile, add-shell/ai/editor/browser→AddWindow, remove→RemoveWindow, browser add/remove/change/reorder-tab→Browser*Tab. Driven through run() against a fake daemon recording the IntentRequest Kind. Read commands (status/doctor) covered by cmd_status_test; intent EFFECTS covered at reducer layer", TestPath: "cmd/projwm/ssot_cli_effect_test.go cmd/projwm/ssot_cli_surface_test.go cmd/projwm/main_test.go", TestName: "TestSSOTCLICommandsSubmitCorrectIntent/TestSSOTCLIExposesBrowserTabOperations/TestRun_DispatchToStatus", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "STATE-DISPLAY", Section: "§3.1 / §5.6 / §10.9 GAP-02", Layer: "L0/L1", Subject: "user-visible per-state display: `projwm status` renders active profile, slot→project assignments, parked + archived projects (正常稼働 / 初期 empty); convergence vocabulary distinguishes 正常 (CONVERGED) from 復旧中/pending (CONVERGING); cockpit tab structure (Slots/Cards/…) via TUI-SNAPSHOT; error state surfaces via [INVARIANT]/[CLOSED]/[MOVED] cards (see ERROR-NOTIF-SURFACES / COMMIT-FAIL-NOTIFY / DRIFT-NOTIFY)", TestPath: "cmd/projwm/cmd_status_test.go", TestName: "TestCmdStatus_Human/TestCmdStatus_JSON/TestStatusConvergenceVocabularyReflectsState", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "GATE-ENFORCE", Section: "§10.2/§10.5 / §10.9 GAP-25", Layer: "L0", Subject: "L3/L4 gate enforcement: ledger L3 owners must sit behind //go:build real_ops and L4 behind //go:build integration (skip-green cannot be claimed as success); L2 owners must NOT be behind those tags. The meta-audit parses each owner file's build tags and fails on violations", TestPath: "internal/ssottest/ssot_l3_l4_gate_audit_test.go", TestName: "TestSSOTLedgerL3OwnersAreGatedByRealOpsTag/TestSSOTLedgerL2OwnersAreNotBehindRealEnvTag", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "COCKPIT-MODES", Section: "§5.4 / §10.9 GAP-12", Layer: "L0/L1", Subject: "cockpit wizard (n → new project/profile prompt, ID typing prefills path, Tab cycles fields, submit-without-ID fails), command palette (Ctrl-P opens/Esc closes, query filters, Enter runs action), and display modes (card push → Proposal mode → returns to prior tab after all cards dismissed; context-aware bottom menu)", TestPath: "cmd/projwm-cockpit/tui/update_test.go", TestName: "TestKey_N_OpensNewProjectPrompt/TestWizard_IDTypingPopulatesPath/TestWizard_TabCyclesFields/TestWizard_SubmitWithoutIDFails/TestPalette_CtrlPOpensAndEscCloses/TestPalette_QueryFilters/TestPalette_EnterRunsAction/TestSubscription_CardAddedEntersProposalMode/TestProposalMode_ReturnsToPreviousTabAfterAllCardsDismissed/TestBottomMenu_ContextAwareOnProfiles", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "STATE-OP-MATRIX", Section: "§3.3 / §10.9 GAP-03", Layer: "L0", Subject: "state×operation availability: the reducer rejects operations that are invalid for the current state — archive/unarchive/add-window/remove-window/delete on an unknown project, switch/delete of an unknown profile, assign/unassign with no active profile, browser-add-tab on unknown project. The §3.3 '復旧中は待機' cell maps to wmMutationLock serialization (see SINGLE-WRITER)", TestPath: "internal/reducer/ssot_l0_state_operation_matrix_test.go", TestName: "TestSSOTState33_ArchiveUnknownProjectRejects/TestSSOTState33_SwitchProfileUnknownRejects/TestSSOTState33_AssignWithNoActiveProfileRejects/TestSSOTState33_AddWindowUnknownProjectRejects/TestSSOTState33_RemoveWindowUnknownProjectRejects/TestSSOTState33_BrowserAddTabRejectsOnUnknownProject", Status: statusCovered, Evidence: evidenceBehavior},

	{ID: "SINGLE-WRITER", Section: "§6.5 / §10.9 GAP-16", Layer: "L1", Subject: "single writer: all WM mutations are serialized through wmMutationLock so concurrent transactions never interleave on the wire (max-in-flight mutation <= 1); external observation events route through reducer.ReactToEvent and never mutate DesiredWorld (only projwmd intents do)", TestPath: "scenarios/transaction_contract_test.go", TestName: "TestTransactionContractS8A_SingleWriter/TestTransactionContractS8E_ExternalEventsDoNotMutateDesired", Status: statusCovered, Evidence: evidenceBehavior},
	...
	{ID: "PRIV-ORPHAN-GC", Section: "§4.4 BR-PRIV-NOSTORE / §1.2 recovery (削除≠アーカイブ)", Layer: "L1", Subject: "Controller post-commit GC: a PrivatePayloadStore ref is Forgotten only when its project is REMOVED from DesiredWorld (DeleteProject{Purge} — the spec purge point), NOT on archive (archive keeps Windows[]+tokens so unarchive→assign restores tabs); unrelated projects' refs are preserved", TestPath: "internal/controller/controller_browser_payload_test.go", TestName: "TestControllerArchiveProject_KeepsBrowserPayloads/TestControllerDeleteProject_ForgetsBrowserPayloads/TestControllerArchiveProject_DoesNotForgetUnrelatedProjectRefs", Status: statusCovered, Evidence: evidenceBehavior},
