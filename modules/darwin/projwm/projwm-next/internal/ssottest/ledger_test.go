...
var ssotLedger = []ledgerItem{
	...
	{ID: "ZED-CONFIG", Section: "§4.4 editor / §10.9 GAP-07", Layer: "L0/L3", Subject: "managed Zed config separation: launch always uses `-n` (new window, not reuse) + `--user-data-dir <private>`; the private settings disable restore_on_startup and auto_install_extensions so the managed Zed never reopens the user's windows or shares extension state. Pre-existing empty-project protection is the L3 owner (TestSpawnEditorEmptyProjectCleanup)", TestPath: "internal/adapter/wm/sigwm_zed_config_test.go internal/adapter/wm/ssot_l3_wm_spec_test.go", TestName: "TestZedManagedSettingsIsolatesFromUserZed/TestZedLaunchArgsForceNewWindowAndPrivateDataDir/TestSpawnEditorEmptyProjectCleanup", Status: statusRealOnly, Evidence: evidenceBehavior},
